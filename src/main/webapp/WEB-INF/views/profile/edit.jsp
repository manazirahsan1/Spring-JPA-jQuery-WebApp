...
<title>Multi Level Authentication: Edit Profile</title>
</head>
<body>
<div class="container">
<div class="row">
			...
			<c:choose>
				<c:when test="${not empty LOGGED_IN_USER_ID}">
					...
					<c:if test="${not empty profile}">
						<spring:url value="/profiles/update/${profile.id}" var="userActionUrl" />
						<form:form class="form-horizontal" method="POST" 
			                modelAttribute="profile" action="${userActionUrl}">
			                
			                <h3 style="text-align:center">Edit Profile</h3><br/>
			        ...
			        <c:if test="${empty profile}">
				        <div class="alert alert-danger">
							<a class="close" onclick="$('.alert').slideUp()">×</a>
								<p><strong>Access denied!!</strong> You have no permission to modify the profile.</p>
						</div>
						<p>You can modify only your own <a href="/profiles" class="btn btn-link">Profile(s)</a>.</p>
			        </c:if>
				</c:when>
				<c:otherwise>
					<div class="alert alert-danger">
					...
					<p>Please <a href="/signin" class="btn btn-link">Sign in</a> to access your own profile(s).</p>
				</c:otherwise>
			</c:choose>
		</div>
	</div>
